...
> Not: `paraner-backend/sql/ai-*.sql` dosyaları (token maliyeti, kullanım sayacı) TARİHSEL
...
- **`destek-hesap-silme-set-null.sql`'in edge bağımlılığı KAPANDI** (2026-07-20): `ticket.user_id`
  ...
  bayrağını unutma** — ya da daha iyisi, ayar artık `paraner-backend/supabase/config.toml`'da
...
- **`paraner-backend/sql/ai-usage-rpc-fix.sql` GEÇERSİZ** — tekrar çalıştırılırsa denetimdeki
