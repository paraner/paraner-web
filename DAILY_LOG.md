...
- **Mobil (≤560px) = resend.com/signup tarzı KOYU (Mehmet isteği):** beyaz kart YOK. `.auth-page` koyu görsel arka plan (cover); `.auth-card`+`.auth-card-form` şeffaf, tam genişlik. Form **koyu temaya** çevrildi (929-971 açık-tema override'ları ≤560'ta geri çevrildi: metin beyaz, başlık/alt dim). Form elemanları (input/sosyal/otp) **DOLGUSUZ/şeffaf** — sadece kenar (`--border-strong`) + metin, görselin üstüne oturur. Wordmark gunmetal→titanyum. Store badge'lere ince kenar. (Switcher + "Devam Et" zaten parlak siyah → koyu zeminde durur.)
