...
- **Sol panel içi boşaltıldı (Mehmet isteği):** sol panel görseli gösteriyor; siyah dolgu kaldırıldı. Beyaz çerçeve korundu. Sağ köşeler yuvarlatıldı (`.auth-visual border-radius: 0 24px 24px 0`) → çentikte kartın beyazı görünür (sol köşelerle simetrik). Panel görseli `.auth-cube { background: url(auth-bg) cover fixed }` → sayfa bg'siyle kesintisiz. `.auth-card` background → `#fff` (köşe çentik beyazı için). NOT: görsel = resend.com/signup'ın `background-auth.jpg`'si (Mehmet indirmiş, aynısı).
- **Mobil (≤560px) = resend.com/signup tarzı KOYU (Mehmet isteği):** beyaz kart YOK. `.auth-page` koyu görsel arka plan (cover); `.auth-card`+`.auth-card-form` şeffaf, tam genişlik. Form **koyu temaya** çevrildi (929-971 açık-tema override'ları ≤560'ta geri çevrildi: metin beyaz, başlık/alt dim). Input/otp **DOLGUSUZ/şeffaf** — sadece kenar + metin, görselin üstüne oturur. Wordmark gunmetal→titanyum. Store badge'lere ince kenar. (Switcher + "Devam Et" zaten parlak siyah → koyu zeminde durur.)
- **Mobil arka plan SABİT + klavye zoom fix (Mehmet isteği):** (1) Arka plan görseli kaydırınca kaymasın → `.auth-page::before` `position:fixed` katman (iOS'ta `background-attachment:fixed` bozuk olduğu için). İçerik üstte kayar, görsel sabit. (2) Klavye açılınca iOS otomatik yakınlaşması → sebebi input `font-size:15px` (<16). `.auth-card-form .field input/select` ≤560'ta `16px` + `.otp-hidden` (OTP gizli input) `16px` → zoom hiç olmaz (geri alma gerekmez).
- **Google butonu yükleme (Mehmet isteği):** basınca giriş beklenirken resend tarzı **3 yanıp sönen nokta** (`.btn-dots`, currentColor → her tema). `signInWithOAuth` yönlenirken logo+yazı yerine `•••`.
- **Mobil sosyal butonlar = resend tarzı (Mehmet isteği):** Google/Apple butonları telefonda **alt alta TAM GENİŞLİK** + hafif koyu dolgu (`rgba(255,255,255,0.05)`) + ince kenar + yuvarlak köşe (14px, pill değil) + logo + ortalı yazı ("Google ile devam et" / "Apple ile devam et"). **Mobilde GIS gizlenir** (`.gsi-wrap display:none`), yerine custom Google butonu (`handleGoogleFallback` → `signInWithOAuth` redirect, `/auth/callback`) → tam kontrol + resend görünümü. GIS tek temayla render edildiği/genişliği px-sabit olduğu için mobil koyuda uymuyordu. `SocialAuth`'a `isMobile` (matchMedia ≤560) eklendi; masaüstü GIS (outline pill) AYNEN korundu. Apple metni isMobile'a göre ("Apple" masaüstü 50% sütunda taşmasın).
